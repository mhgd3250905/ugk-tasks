...
### 组装 timeWindow 对象(第6步传给 collect-and-write.mjs 用)

worker 把扁平字段组装成 timeWindow 对象,第 6 步作为 `--timeWindow` 参数(JSON 字符串,单引号包裹)传给 `collect-and-write.mjs`:
```json
...
**关键认知:worker 是 LLM agent,不是 node 进程** —— 唯一能落盘的是 `write` tool,它会逐 token 输出整个 JSON(实测 33KB 量级花 +124s,且 LLM 手拼 JSON 易转义错)。所以**严禁用 `write` tool 输出 results**。

正确做法:bash 调 `scripts/collect-and-write.mjs`,它自己直连 CDP 取 rows + `JSON.stringify` + `writeFileSync`(确定性、<1s、JSON 保证合法)。worker 只把 timeWindow、benchmark、URL 等**小参数**传进去(rows 内容完全不经过 worker):

```bash
node "$TASK_DIR/scripts/collect-and-write.mjs" \
  --rawQuery "<原始 keyword>" \
  --normalizedKeyword "<keyword>" \
  --timeWindow '<组装好的 timeWindow JSON,见"输入"段>' \
  --cutoffIso "<startIso>" \
  --searchUrl "<https://x.com/search?q=...&f=live>" \
  --benchmark '<组装好的 benchmark JSON,见下>' \
  --output "$TASK_OUTPUT_DIR/x_search_results.json"
```

**timeWindow 参数**:把"输入"段的扁平字段组装成对象后 `JSON.stringify`,作为 `--timeWindow` 的值(单引号包裹):
`{"raw":"<timePhrase>","mode":"<timeMode>","amount":<timeAmount>,"unit":"<timeUnit>","startIso":"<startIso>","endIso":"<endIso>","canonical":"<canonical>"}`

**benchmark 参数**(worker 在调用前组装好,基于第 5 步 anchor-scroll 返回值 + 自己算的派生指标):
```json
{
  "stopReason": "<第5步返回>",
  "cutoffReached": <bool>,
  ...
  "validRows": <N>,
  "filteredRows": <N>,
  "rowsReturned": <worker 预估或留 0,脚本会用实际 rows.length 覆盖>,
  "validRate": <0-1>,
  "keywordMatchRate": <0-1>,
  "totalRunMs": <N>
}
```
`rowsReturned` 字段脚本会用实际取到的 rows 数量覆盖,worker 传任意值都行。

脚本内部:循环 `chrome_cdp evaluate dump-result.js`(offset+=100,source=rows,复用当前 X tab)取全量 rows → 字段映射(content→text、authorName→author、authorHandle→handle)→ 组装 envelope → `JSON.stringify` → `writeFileSync` → round-trip 自检。产出结构:

```json
{
  "rawQuery": "<原始 keyword>",
  "normalizedKeyword": "<keyword>",
  "timeWindow": { "raw": "...", "mode": "...", "amount": <N>, "unit": "...", "startIso": "...", "endIso": "...", "canonical": "..." },
  "cutoffIso": "<startIso>",
  "retrievedAt": "<ISO>",
  ...
  "benchmark": { "stopReason":"...", "cutoffReached":<bool>, "score":<N>, "grade":"...", "anchorScrolls":<N>, "maxSteps":<N>, "rowsInspected":<N>, "validRows":<N>, "filteredRows":<N>, "rowsReturned":<N>, "validRate":<0-1>, "keywordMatchRate":<0-1>, "totalRunMs":<N> },
  "results": [ { "postedAt": "...", "text": "...(完整原文)...", "url": "...", "author": "...", "handle": "..." } ]
}
```

`source` 默认 `'rows'`(时间窗内 + 关键词命中全集),够交付。用户若显式要含范围外原始数据,加 `--source allRows` 参数。

**🚫 严禁:** 用 `write` tool 输出 results(慢 + JSON 易错)。
