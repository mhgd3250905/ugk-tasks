// write-output.mjs — ins-search 的最终产物组装脚本。
// ponytail: 替代 worker 用 write tool 逐 token 输出 JSON(慢 + JSON 易错)。
// 数据已在 `_filtered.json`(filter-lib selectRecentRelevantPosts 产物),本脚本纯文件操作,不需 CDP。
// 读 _filtered.json → postUrl→url 映射 → 包 envelope → writeFileSync → round-trip 自检。
//
// 用法(单行 bash,worker 调):
//   node "$TASK_DIR/scripts/write-output.mjs" \
//     --keyword "<原始 keyword>" --timePhrase "<timePhrase>" --days <N> \
//     --queryUrl "<URL>" --benchmark '<benchmark JSON,含 seeds[]>' \
...
function mapRow(r) {
	// _filtered.json row 字段:filter-lib normalizeInstagramPost 输出 postUrl,
	// 但 worker 有时跳过 normalize 直接存(字段会是 url)。兼容两者,避免 worker 重跑。
	const url = str(r.postUrl || r.url);
	return {
		postedAt: str(r.postedAt),
		author: str(r.author),
		titleDerived: str(r.titleDerived),
		caption: str(r.caption),
		likeCount: num(r.likeCount),
		commentCount: num(r.commentCount),
		matchReason: str(r.matchReason),
		matchedBy: str(r.matchedBy),
		url,
	};
}
...
async function main() {
	...
	const envelope = {
		...
		benchmark: {
			seedsScanned: num0(benchmark.seedsScanned),
			totalDiscovered: num0(benchmark.totalDiscovered),
			detailFetched: num0(benchmark.detailFetched),
			filteredRows: filtered.length,
			stopReason: str(benchmark.stopReason),
			seeds: Array.isArray(benchmark.seeds) ? benchmark.seeds : [],
		},
	...
	console.log(`ins write-output: ${filtered.length} rows, ${text.length} bytes → ${output}`);
}
